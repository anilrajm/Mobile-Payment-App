...
class OperatorPlanProvider with ChangeNotifier {
  bool _isLoading = false;
  ...
  GetOperatorPlan get operatorPlanModel => _operatorPlanModel;
...
class OperatorBillProvider with ChangeNotifier {
  late GetOperatorBill _operatorBillModel;

  GetOperatorBill get operatorBillModel => _operatorBillModel;
...
class BusinessTypeProvider {
  late BusinessType businessType;
  late String _deviceId;

  Future<BusinessType> fetchBusinessTypes() async {
    try {
      AndroidDeviceInfo androidInfo = await DeviceInfoPlugin().androidInfo;
      _deviceId = androidInfo.id.toString();
      businessType = await APIManagement().fetchBusinessTypes(
        deviceId: _deviceId,
      );
    } catch (e) {
      if (kDebugMode) {
